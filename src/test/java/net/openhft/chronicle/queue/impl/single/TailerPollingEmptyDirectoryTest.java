...
public class TailerPollingEmptyDirectoryTest {
    ...
    public void shouldNotGenerateExcessGarbage() throws Exception {
        try (final SingleChronicleQueue queue = createQueue()) {
            queue.path.mkdirs();
            assertThat(queue.path.list().length, is(0));
    ...
    private SingleChronicleQueue createQueue() {
        return SingleChronicleQueueBuilder.binary(
                DirectoryUtils.tempDir(TailerPollingEmptyDirectoryTest.class.getName())).
                testBlockSize().reduceTailerPollingGarbage(true).
                build();
    }
}